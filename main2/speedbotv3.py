"""
speedbotv3.py — IIT Pokerbots 2026 "Sneak Peek Hold'em" Bot v5

Combines speedbotv2's full playing strategy with the auction exploit.

Playing Strategy (from speedbotv2):
...
Auction Strategy (EXPLOIT — intercepted opponent bid):
  ...
  • Weak hand  (eq < 0.45) → Bid opp-1 (DRAIN MODE: lose, pay 0, drain opp)
  • Medium-strong (eq≥0.45) → Bid opp+1 (WIN MODE: see their card, pay opp_bid)

  Edge cases: 0 chips, can't outbid, BB first-bidder, interception failure, ties.
"""
...
class Player(BaseBot):
    ...
    def __init__(self) -> None:
        ...
        # Auction outcome tracking (FIX 3 from v2)
    ...
    # ------------------------------------------------------------------
    # Auction  (EXPLOIT — exact same logic as exploit_bot)
    # ------------------------------------------------------------------
    def _bid(self, cs: PokerState) -> int:
        ...
        if cs.is_bb:
            pot = cs.pot
            # Use same aggressive strategy as speedbotv2 to stay competitive
        ...
        # MEDIUM-STRONG (eq ≥ 0.45): WIN MODE
    ...
    # ------------------------------------------------------------------
    # Preflop sub-strategy  (unchanged from speedbotv2)
    # ------------------------------------------------------------------

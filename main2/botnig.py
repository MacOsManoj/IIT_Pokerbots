...
Based on temp_drainv3.py with enhanced premium hand strategy and pot odds decision making.

Key Changes from v3:
  - Premium hands: probe-then-bluff instead of all-in preflop
    - Raise 5-10x BB (100-200) preflop
    - Build pot gradually postflop (50-75% pot bets)
    - Cut losses on bad boards
    - Bluff probes at same sizing when opponent fold rate > 50%
  - Thresholds increased by 20% for better pot odds calls
  - Pot odds formula for calling: required_equity = to_call / (pot + to_call) + 0.05 margin
"""
...
# Bluff bet sizing per street (fraction of pot)
BLUFF_BET_FRACTION = {
    "flop": (0.30, 0.40),       # 30-40% pot on flop
    "turn": (0.35, 0.45),       # 35-45% pot on turn
    "river": (0.40, 0.50),      # 40-50% pot on river
}

# ---------------------------------------------------------------------------
# PREMIUM PROBE-THEN-BLUFF CONFIGURATION
# ---------------------------------------------------------------------------
PREMIUM_PREFLOP_RAISE_MIN = 100   # 5x BB
PREMIUM_PREFLOP_RAISE_MAX = 200   # 10x BB
PREMIUM_POSTFLOP_BET_MIN = 0.50   # 50% pot
PREMIUM_POSTFLOP_BET_MAX = 0.75   # 75% pot
PREMIUM_BAD_BOARD_CUTOFF = 0.55   # Below this equity, cut losses
PREMIUM_BLUFF_FOLD_THRESHOLD = 0.50  # Bluff at premium sizing if opp folds > 50%
...
class Player(BaseBot):
    ...
    def __init__(self) -> None:
        ...
        # ═══════════════════════════════════════════════════════════════════
        # PREMIUM PROBE-THEN-BLUFF STATE (NEW in v4)
        # ═══════════════════════════════════════════════════════════════════
    ...
    # ------------------------------------------------------------------
    # PREMIUM PROBE BET: Gradual pot building for premium hands (NEW in v4)
    # ------------------------------------------------------------------
    ...
    def _trash_action(self, cs: PokerState):
        ...
        if to_call > 0:
            ...
            # Chip threshold check (bypass if we have strong value equity)
            if total_street_cost > threshold and exact_eq < value_bet_floor:
                return ActionFold() if cs.can_act(ActionFold) else ActionCheck()
            ...
            # Calculate pot size from previous actions to detect overbet correctly. 
            # cs.pot contains my wager and opp wager of current street.
            previous_pot = max(cs.pot - cs.my_wager - cs.opp_wager, 1)
            bet_to_pot_ratio = to_call / previous_pot

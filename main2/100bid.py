...
class Player(BaseBot):
    ...
    def __init__(self) -> None:
        ...
        # Track opponent betting aggression cumulative across entire hand
        self._opp_hand_bet_count = 0        # How many times opponent bet/raised this hand (cumulative)
        self._opp_total_wagered = 0         # Track opponent's total wager to detect new bets
    ...
    def on_hand_start(self, game_info: GameInfo, cs: PokerState) -> None:
        ...
        self._opp_hand_bet_count = 0
        self._opp_total_wagered = 0
    ...
    def _get_call_equity_threshold(self, bet_to_pot_ratio: float) -> float:
        """
        Get dynamic equity threshold for calling based on:
        - Number of opponent bets/raises this street
        - Bet sizing relative to pot
        
        More aggression from opponent = higher equity needed to call.
        """
        # Base threshold starts at 0.40
        base_threshold = 0.40
        
        # Add 0.05 for each opponent bet in the hand (cumulative - they keep betting = stronger range)
        aggression_penalty = self._opp_hand_bet_count * 0.05
        
        # Add penalty based on bet size (bigger bets = more confident opponent)
        # bet_to_pot_ratio: 0.5 (half pot) -> +0.03, 1.0 (pot) -> +0.06, 2.0+ (overbet) -> +0.12+
        sizing_penalty = min(bet_to_pot_ratio * 0.06, 0.15)  # Cap at 0.15
        
        # Total threshold (cap at 0.80 - don't be too tight)
        threshold = min(base_threshold + aggression_penalty + sizing_penalty, 0.80)
    ...
    def _bid(self, cs: PokerState) -> int:
        ...
        # ════════════════════════════════════════════════════════
        # BB (first bidder): bid small for trash hands
        # ════════════════════════════════════════════════════════
        if cs.is_bb:
            # Trash hand: bid 1-2 (show minimum interest)
    ...
    def get_move(self, game_info: GameInfo, cs: PokerState):
        # Track street changes and reset street commitment only
        if cs.street != self._current_street:
            self._current_street = cs.street
            self._street_committed = 0
        
        # Detect if opponent made a new bet/raise (cumulative across entire hand)
        current_opp_total = getattr(cs, 'opp_wager', 0) + getattr(cs, 'opp_pip', 0)
        if current_opp_total > self._opp_total_wagered:
            self._opp_hand_bet_count += 1
            self._opp_total_wagered = current_opp_total
    ...
    def _trash_action(self, cs: PokerState):
        ...
        if to_call > 0:
            ...
            # Dynamic equity threshold based on opponent aggression and bet sizing
            call_threshold = self._get_call_equity_threshold(bet_to_pot_ratio)
            
            # Call if equity exceeds dynamic threshold
            if exact_eq >= call_threshold:
                if cs.can_act(ActionCall):
                    return ActionCall()
            
            # Poor equity relative to opponent's aggression, fold
            return ActionFold() if cs.can_act(ActionFold) else ActionCheck()

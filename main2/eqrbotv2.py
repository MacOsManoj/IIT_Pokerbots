"""
eqrbotv2.py — Sneak Peek Hold'em Bot (IIT Pokerbots 2026)
Strategy: Equity Realization (EQR) + Auction Exploit + Safeguard Mode

═══════════════════════════════════════════════════════════════
EQUITY CALCULATION
═══════════════════════════════════════════════════════════════
  Pre-flop : PREFLOP_EQ lookup table (pre-computed 5000-sim MC
             for all 169 canonical hands, keyed as 'AKs'/'AKo'/'AA')
  Flop     : Monte Carlo, 200 iterations  (_mc_eq)
  Turn     : Monte Carlo, 250 iterations
  River    : Monte Carlo, 350 iterations
  When we WON auction: opponent's revealed card is passed to
             _mc_eq as opp_known, constraining villain's range.

═══════════════════════════════════════════════════════════════
EQR MULTIPLIER  (applied AFTER raw equity, BEFORE adjustments)
═══════════════════════════════════════════════════════════════
  realized_equity = raw_equity × eqr
  eqr starts at 1.0 and is modified by:
    Position   : IP  → +0.08 × street_weight
                 OOP → −0.06 × street_weight
    Suitedness  : suited hole cards → +0.05 × sw
    Pairs       : TT+ → +0.04 × sw  |  22–55 → −0.06 × sw
    Connectors  : gap=0 → +0.04 × sw  |  gap≥3 → −0.03 × sw
    Broadway    : both cards T+ → +0.03 × sw
    Ace kicker  : hi rank = Ace → +0.02 × sw
    Street weights (sw): pre-flop=1.0, flop=0.6, turn=0.3, river=0.0
  Final eqr is clamped to [0.65, 1.25].

═══════════════════════════════════════════════════════════════
INFORMATION EXPLOITATION  (applied after EQR)
═══════════════════════════════════════════════════════════════
  If we WON the auction and opponent's card is revealed:
    Paired board OR flush made  → equity −0.05  (opp_card_strong)
    High card (T+) + flush draw → equity −0.03  (opp_card_strong)
    Low card (≤5) not on board  → equity +0.04  (opp_card_weak)
    Mid card (≤8) not on board  → equity +0.02  (opp_card_weak)
  If OPPONENT won the auction   → equity −0.03 (flat penalty)

  Board texture (after info exploit):
    Wet board + OOP + equity 0.45–0.65 → equity −0.05

═══════════════════════════════════════════════════════════════
FACING-BET DISCOUNT  (post-flop only, applied last)
═══════════════════════════════════════════════════════════════
  bet_frac = cost_to_call / (pot − cost_to_call)
  base discount = min(0.18, bet_frac × 0.14)
  If opponent WON auction AND bet_frac > 0.25:
      discount ×= 1.7, capped at 0.20
  If opp_card_strong:
      discount += 0.05
  equity −= discount  (floor 0)

═══════════════════════════════════════════════════════════════
AUCTION LOGIC  (ExploitRunner intercepts opponent bid first)
═══════════════════════════════════════════════════════════════
  BB (first bidder) — CONSERVATIVE, saves chips:
    eq > 0.80 → bid 25–40% pot  |  eq > 0.68 → 14–25% pot
    eq > 0.55 → 6–14% pot       |  eq < 0.30 → 0–2% pot
    eq < 0.40 → 1–4% pot        |  else → uncertainty × pot × 0.18
    All bids capped at min(7% stack, 45% pot) + ±13% noise.

  SB (second bidder) — EXPLOITS opponent's known bid:
    opp_bid unknown  → bid 10% pot if eq > 0.5, else 0
    opp_bid == 0     → bid 1 (guaranteed free info)
    eq < 0.35        → bid opp_bid − 1 (DRAIN mode: let them win, bleed chips)
    eq ≥ 0.35        → bid opp_bid + 1 (WIN mode: pay minimum to secure the card)

═══════════════════════════════════════════════════════════════
SAFEGUARD MODE  (entered when winning)
═══════════════════════════════════════════════════════════════
  Activated if:
    bankroll > (rounds_left × 15) + 10   [guaranteed win threshold]
    OR bankroll > 500                     [solid score threshold]
  Effect: equity −= 0.06, preflop equity −= additional 0.05
  Goal: play hyper-defensively to lock in the positive bankroll.

═══════════════════════════════════════════════════════════════
POSTFLOP DECISION BUCKETS  (on adjusted equity)
═══════════════════════════════════════════════════════════════
  > 0.84  PREMIUM  : bet 15–30% pot (bleed small, keep them in)
  > 0.70  STRONG   : if facing bet → call; else → bet 33–50% pot
                     (freq: 50% if opp weak card, else 40%)
  > 0.57  MEDIUM   : facing bet → call if pot_odds < equity − 0.06
                     no bet → probe IP 30% of the time (28–40% pot)
  > 0.45  MARGINAL : facing bet → call if pot_odds< equity−0.08 AND r<0.30
                     no bet → always check
  > 0.33  WEAK     : facing bet → check if possible, else fold
                     no bet → bluff-stab if opp_card_weak+IP+dry board (8+bluff_mod%)
  ≤ 0.33  TRASH    : facing bet → check/fold; no bet → pure bluff (3+bluff_mod%)

═══════════════════════════════════════════════════════════════
PREFLOP DECISION
═══════════════════════════════════════════════════════════════
  IP  (SB): eq > 0.60 → raise 2.5×BB  |  eq > 0.40 → call  |  else → fold
  OOP (BB): to_call=0 → raise if eq>0.62, else check
            facing raise: eq > 0.70 → 3-bet 2.5×pot
                          eq > 0.38 → call  |  else → fold
  [Safeguard: thresholds tighten by −0.05]

═══════════════════════════════════════════════════════════════
OPPONENT MODEL  (updates each action)
═══════════════════════════════════════════════════════════════
  opp_fold_pct : folds / opportunities (needs 15+ samples)
  opp_agg      : raises / total actions (needs 15+ samples)
  bluff_mod    : +0.04 if fold_pct > 0.50  |  −0.04 if fold_pct < 0.25
                 (activates after 50 hands)
"""
...
class Player(BaseBot):
    ...
    def _eqr(self, hand: list[str], is_ip: bool, street: str) -> float:
        """
        EQR multiplier: realized_equity = raw_equity x eqr

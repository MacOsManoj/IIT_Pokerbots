"""
simpleoppbot.py — All-In-or-Fold Strategy (IIT Pokerbots 2026)

Strategy:
  Phase 1 (Hunt): Go ALL-IN preflop on premium hands (equity >= 0.75).
                   Fold everything else.
  Phase 2 (Lock): Once bankroll >= BANKROLL_TARGET, fold/check every hand
                   to lock in the profit.

Math:
  - Folding every hand costs ~15,000 over 1000 rounds (500 BB×20 + 500 SB×10)
  - Each all-in win nets ~5,000 chips
  - ~70 premium hands in 1000 rounds → need only ~5 wins to cover fold cost
  - Once bankroll >= 15,000, we can fold everything and still finish positive
"""
...
# Configuration
# ---------------------------------------------------------------------------
ALLIN_THRESHOLD = 0.75    # Go all-in if preflop equity >= this
BANKROLL_TARGET = 25000   # Once reached, fold everything to lock in profit
...
class Player(BaseBot):
    ...
    def get_move(self, game_info: GameInfo, cs: PokerState):
        # ==============================================================
        # PHASE 2 — LOCK-IN MODE: bankroll target reached, fold everything
        # ==============================================================
        ...
        # --- Auction: always bid 0 (we don't need info) ---
        if cs.street == "auction":
            return ActionBid(0)

        # --- Preflop: all-in on premium, fold/check everything else ---
        if cs.street == "pre-flop":
            eq = self._pf_eq(cs.my_hand)

            if eq >= ALLIN_THRESHOLD:
            ...
            # Not premium: fold (but check if free as BB)
            if cs.cost_to_call == 0:
                return ActionCheck()
            return ActionFold() if cs.can_act(ActionFold) else ActionCheck()

        # --- Postflop: check/fold (we should rarely get here) ---
        # We only reach postflop if:
        #   - We were BB and checked preflop (non-premium hand)
        #   - Or we went all-in and both players are already all-in
        if cs.can_act(ActionCheck):
            return ActionCheck()
        return ActionFold() if cs.can_act(ActionFold) else ActionCheck()
...
if __name__ == "__main__":
    from pkbot.runner import run_bot

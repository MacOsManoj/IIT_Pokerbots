"""
tdv5.py — Adaptive Bot with Thin Value Sizing and Passive Opponent Exploitation
...
# ---------------------------------------------------------------------------
BLUFF_MIN_FOLD_PCT = 0.60       # Min fold % to activate bluffing
BLUFF_MIN_SAMPLES = 8           # Min raise data points before bluffing
BLUFF_MAX_FREQUENCY = 0.20      # Cap bluff frequency at 20% of eligible hands
BLUFF_MAX_INVESTMENT = 100      # Max chips to risk per bluffing hand
...
# ---------------------------------------------------------------------------
# PREMIUM THIN VALUE CONFIGURATION (v5 - changed from probe-then-bluff)
# ---------------------------------------------------------------------------
PREMIUM_PREFLOP_RAISE_MIN = 100   # 5x BB
PREMIUM_PREFLOP_RAISE_MAX = 200   # 10x BB
PREMIUM_POSTFLOP_BET_MIN = 0.25   # was 0.50 - now thin value sizing
PREMIUM_POSTFLOP_BET_MAX = 0.40   # was 0.75 - smaller to get more calls
...
class Player(BaseBot):
    ...
    def _update_street_thresholds(self) -> None:
        ...
        if aggression_factor < PASSIVE_AGGRESSION_THRESHOLD:
            ...
            # Preflop: up to 1.5x (max 180)
            preflop_mult = 1.0 + (0.5 * passivity)
            self._dynamic_preflop_threshold = min(180, int(BASE_PREFLOP_THRESHOLD * preflop_mult))
            
            # Flop: up to 1.4x (max 200)  
            flop_mult = 1.0 + (0.4 * passivity)
            self._dynamic_flop_threshold = min(200, int(BASE_FLOP_THRESHOLD * flop_mult))
            
            # Turn: up to 1.35x (max 220)
            turn_mult = 1.0 + (0.35 * passivity)
            self._dynamic_turn_threshold = min(220, int(BASE_TURN_THRESHOLD * turn_mult))
            
            # River: up to 1.3x (max 240)
            river_mult = 1.0 + (0.3 * passivity)
            self._dynamic_river_threshold = min(240, int(BASE_RIVER_THRESHOLD * river_mult))
        else:

import random
import time
import socket
import argparse

import eval7

from pkbot.actions import ActionFold, ActionCall, ActionCheck, ActionRaise, ActionBid
from pkbot.states import GameInfo, PokerState
from pkbot.base import BaseBot
from pkbot.runner import Runner, run_bot, parse_args

# --- Precomputed Preflop Equities (snippet from eqr_bot.py) ---
PREFLOP_EQ: dict[str, float] = {
    "AA": 0.852183, "KK": 0.824196, "QQ": 0.799191, "JJ": 0.774431,
    "TT": 0.750071, "99": 0.720453, "88": 0.691701, "AKs": 0.670392,
    "77": 0.662712, "AQs": 0.662197, "AJs": 0.653904, "AKo": 0.653225,
    "ATs": 0.646025, "AQo": 0.644168, "AJo": 0.635557, "KQs": 0.634290,
    "66": 0.633214, "A9s": 0.627679, "ATo": 0.627385, "KJs": 0.625471,
    "A8s": 0.619500, "KTs": 0.617886, "KQo": 0.614722, "A7s": 0.609758,
    "A9o": 0.607503, "KJo": 0.605522, "55": 0.603176, "QJs": 0.602634,
    "K9s": 0.600114, "A6s": 0.599297, "A5s": 0.599217, "A8o": 0.598766,
    "KTo": 0.597529, "QTs": 0.594641, "A4s": 0.590452, "A7o": 0.588179,
    "K8s": 0.583135, "A3s": 0.582202, "QJo": 0.581244, "K9o": 0.578094,
    "A6o": 0.577104, "A5o": 0.577031, "Q9s": 0.576858, "K7s": 0.575437,
    "JTs": 0.575282, "A2s": 0.574308, "QTo": 0.573167, "44": 0.570136,
    "A4o": 0.566985, "K6s": 0.566398, "K8o": 0.560125, "Q8s": 0.559834,
    "A3o": 0.558116, "K5s": 0.558004, "J9s": 0.556544, "Q9o": 0.553631,
    "JTo": 0.552446, "K7o": 0.551691, "A2o": 0.548994, "K4s": 0.548895,
    "Q7s": 0.543058, "K6o": 0.541934, "K3s": 0.540632, "T9s": 0.540342,
    "J8s": 0.540195, "33": 0.537315, "Q6s": 0.536214, "Q8o": 0.536113,
    "K5o": 0.533013, "J9o": 0.532693, "K2s": 0.531965, "Q5s": 0.527635,
    "K4o": 0.523368, "T8s": 0.523207, "J7s": 0.523185, "Q4s": 0.518402,
    "Q7o": 0.517932, "T9o": 0.515265, "J8o": 0.514831, "K3o": 0.514342,
    "Q6o": 0.510226, "Q3s": 0.510111, "98s": 0.508049, "T7s": 0.506383,
    "J6s": 0.506014, "K2o": 0.505214, "22": 0.503355, "Q2s": 0.501650,
    "Q5o": 0.501567, "J5s": 0.499679, "T8o": 0.497304, "J7o": 0.496586,
    "Q4o": 0.491312, "97s": 0.490826, "J4s": 0.490709, "T6s": 0.489450,
    "J3s": 0.482458, "Q3o": 0.482211, "98o": 0.480816, "87s": 0.479239,
    "T7o": 0.479023, "J6o": 0.478494, "96s": 0.474308, "J2s": 0.473701,
    "Q2o": 0.473027, "T5s": 0.472144, "J5o": 0.471488, "T4s": 0.465563,
    "97o": 0.462920, "86s": 0.462199, "J4o": 0.461736, "T6o": 0.460856,
    "95s": 0.457090, "T3s": 0.457052, "76s": 0.453805, "J3o": 0.452673,
    "87o": 0.450608, "T2s": 0.448209, "85s": 0.445399, "96o": 0.444856,
    "J2o": 0.443324, "T5o": 0.442603, "94s": 0.438509, "75s": 0.436531,
    "T4o": 0.435045, "93s": 0.432652, "86o": 0.432537, "65s": 0.431193,
    "84s": 0.427003, "95o": 0.426803, "T3o": 0.425601, "92s": 0.423873,
    "76o": 0.423251, "74s": 0.418559, "T2o": 0.416894, "54s": 0.414434,
    "85o": 0.414433, "64s": 0.413518, "83s": 0.408648, "94o": 0.406452,
    "75o": 0.405143, "82s": 0.402756, "73s": 0.400491, "93o": 0.399820,
    "65o": 0.399579, "53s": 0.397308, "63s": 0.395181, "84o": 0.394336,
    "92o": 0.391195, "43s": 0.386398, "74o": 0.385508, "72s": 0.381638,
    "54o": 0.381554, "64o": 0.380063, "52s": 0.378516, "62s": 0.376704,
    "83o": 0.374886, "42s": 0.368367, "82o": 0.368321, "73o": 0.365887,
    "53o": 0.362707, "63o": 0.360540, "32s": 0.359599, "43o": 0.351484,
    "72o": 0.346146, "52o": 0.343080, "62o": 0.341154, "42o": 0.332130,
    "32o": 0.323179,
}

RANK_ORDER = "23456789TJQKA"
RANK_VAL = {r: i for i, r in enumerate(RANK_ORDER)}

_ANY_TWO_STR = ",".join(
    r1 + s1 + r2 + s2
    for i, (r1, s1) in enumerate([(r, s) for r in '23456789TJQKA' for s in 'cdhs'])
    for r2, s2 in [(r, s) for r in '23456789TJQKA' for s in 'cdhs'][i + 1:]
)
_ANY_TWO_RANGE = eval7.HandRange(_ANY_TWO_STR)
_ONE_CARD_CACHE = {}

def _hkey(cards: list[str]) -> str:
    if len(cards) != 2:
        return ""
    r1, s1 = cards[0][0], cards[0][1]
    r2, s2 = cards[1][0], cards[1][1]
    v1, v2 = RANK_VAL.get(r1, 0), RANK_VAL.get(r2, 0)
    if v1 < v2:
        r1, r2 = r2, r1
        s1, s2 = s2, s1
    if v1 == v2:
        return r1 + r2
    return r1 + r2 + ("s" if s1 == s2 else "o")



class ExploitRunner(Runner):
    '''
    Modified runner that intercepts 'A<amount>' packets to extract
    opponent's auction bid BEFORE the main Runner processes them.
    '''
    def __init__(self, pokerbot, socketfile):
        super().__init__(pokerbot, socketfile)

    def receive(self):
        while True:
            packet = self.socketfile.readline().strip().split(' ')
            if not packet:
                break
            for clause in packet:
                if clause and len(clause) > 1 and clause[0] == 'A':
                    try:
                        bid_amount = int(clause[1:])
                        if hasattr(self.pokerbot, '_receive_opponent_bid'):
                            self.pokerbot._receive_opponent_bid(bid_amount)
                    except (ValueError, IndexError):
                        pass
            yield packet



class Player(BaseBot):
    def __init__(self) -> None:
        random.seed(time.time() + id(self))
        self.opponent_last_bid = None
        self._auction_decision_made = False
        self._we_won_auction = False
        self._opp_won_auction = False
        self.opp_folds = 0
        self.opp_fold_opps = 0
        self.total_hands = 0
        self.raise_count = 0
        self.current_street = ""

    @property
    def opp_fold_pct(self) -> float:
        return self.opp_folds / self.opp_fold_opps if self.opp_fold_opps > 15 else 0.45

    def on_hand_start(self, game_info: GameInfo, cs: PokerState) -> None:
        self.opponent_last_bid = None
        self._auction_decision_made = False
        self._we_won_auction = False
        self._opp_won_auction = False
        self.raise_count = 0
        self.current_street = ""

    def on_hand_end(self, game_info: GameInfo, cs: PokerState) -> None:
        pass

    def _receive_opponent_bid(self, bid_amount: int) -> None:
        if not self._auction_decision_made:
            self.opponent_last_bid = bid_amount

    def _exact_equity(self, hand: list[str], board: list[str], opp_known: list[str] = None) -> float:
        h = [eval7.Card(c) for c in hand]
        b = [eval7.Card(c) for c in board]
        if opp_known and len(opp_known) == 2:
            villain = eval7.HandRange("".join(opp_known))
        elif opp_known and len(opp_known) == 1:
            t = opp_known[0]
            if t in _ONE_CARD_CACHE:
                villain = _ONE_CARD_CACHE[t]
            else:
                dk = [r + s for r in '23456789TJQKA' for s in 'cdhs']
                villain = eval7.HandRange(",".join(t + c for c in dk if c != t))
                _ONE_CARD_CACHE[t] = villain
        else:
            villain = _ANY_TWO_RANGE
        return float(eval7.py_hand_vs_range_exact(h, villain, b))

    def _pf_eq(self, hand: list[str]) -> float:
        return PREFLOP_EQ.get(_hkey(hand), 0.47)

    def _equity(self, cs: PokerState) -> float:
        if cs.street == "pre-flop":
            return self._pf_eq(cs.my_hand)
        ok = cs.opp_revealed_cards or None
        return self._exact_equity(cs.my_hand, cs.board, opp_known=ok)

    def _bid(self, cs: PokerState) -> int:
        if cs.my_chips <= 0:
            return 0

        eq = self._equity(cs)
        
        self._auction_decision_made = True

        if cs.is_bb:
            pot = cs.pot
            if eq > 0.80: iv = pot * random.uniform(0.25, 0.40)
            elif eq > 0.68: iv = pot * random.uniform(0.14, 0.25)
            elif eq > 0.55: iv = pot * random.uniform(0.06, 0.14)
            elif eq < 0.30: iv = pot * random.uniform(0.00, 0.02)
            elif eq < 0.40: iv = pot * random.uniform(0.01, 0.04)
            else:
                unc = max(0.0, 1.0 - (2.0 * abs(eq - 0.5)) ** 1.4)
                iv = unc * pot * 0.18
            iv = min(iv, cs.my_chips * 0.07, pot * 0.45)
            noise = random.uniform(0.87, 1.13)
            iv *= noise
            return int(max(0, min(iv, cs.my_chips)))

        opp_bid = self.opponent_last_bid
        if opp_bid is None:
            if eq > 0.5: return min(int(cs.pot * 0.10), cs.my_chips)
            return 0
        if opp_bid == 0:
            return min(1, cs.my_chips)
            
        WEAK_AUCTION_EQ = 0.35
        if eq < WEAK_AUCTION_EQ:
            drain_bid = max(opp_bid - 1, 0)
            return min(drain_bid, cs.my_chips)
            
        win_bid = opp_bid + 1
        win_bid = min(win_bid, cs.my_chips)
        if win_bid <= opp_bid:
            return cs.my_chips
        return win_bid

    def _do_raise(self, cs: PokerState, amt: float):
        amt = int(amt)
        # If we just want to add 0, it means call or check
        if amt <= 0:
            return ActionCall() if cs.can_act(ActionCall) else ActionCheck()
            
        mn, mx = cs.raise_bounds
        # raise_bounds refers to the total post-action contribution. 
        # The amount we need to reach is my_contribution + cost_to_call + amt
        target = cs.my_wager + cs.cost_to_call + amt
        
        if not cs.can_act(ActionRaise):
            return ActionCall() if cs.can_act(ActionCall) else ActionCheck()
            
        return ActionRaise(min(mx, max(mn, target)))

    def _get_action(self, cs: PokerState, winRate: float):
        amount = cs.cost_to_call
        pot = cs.pot
        pot_minus_amount = pot - amount
        
        # Kelly Criterion base parameters
        pf_edge = max(0.0, winRate - 0.45)
        post_edge = max(0.0, winRate - 0.50)
        opp_agg_discount = max(0.2, 1.0 - (amount / (pot + 0.01)))

        # ------------------------------------------------------------------
        # UNIVERSAL POT-ODDS SAFETY VALVE
        # ------------------------------------------------------------------
        pot_odds = amount / (pot + 0.0001) if amount > 0 else 0.0
        
        if amount > 0:
            # We measure how massive their bet is relative to the pot *before* they bet
            # pot_minus_amount is roughly the pot size before this action
            base_pot = pot_minus_amount if pot_minus_amount > 0 else 1.0
            bet_size_ratio = amount / base_pot
            
            # Base required ratio is 1.4, but scales up heavily if they overbet
            required_ratio = 1.4
            if bet_size_ratio >= 1.0: # Pot-sized bet or more
                required_ratio = 1.8
            if bet_size_ratio >= 2.0: # 2x Pot bet
                required_ratio = 2.2
            if bet_size_ratio >= 5.0: # Massive all-in overbet
                required_ratio = 2.8
                
            required_win_rate = pot_odds * required_ratio
            if winRate < required_win_rate:
                return ActionFold() if cs.can_act(ActionFold) else ActionCheck()

        if cs.street == "pre-flop":
            if winRate < 0.48: # Optimized 0.48
                if winRate >= (amount * 1.4) / (amount + pot + 0.0001):
                    return ActionCall() if cs.can_act(ActionCall) else ActionCheck()
                else:
                    return ActionFold() if cs.can_act(ActionFold) else ActionCheck()
                    
            elif winRate <= 0.65: # Optimized 0.65
                if amount > pot_minus_amount * 3.0:
                    return ActionFold() if cs.can_act(ActionFold) else ActionCheck()
                else:
                    raise_add = (pf_edge * 2.5) * opp_agg_discount * pot 
                    return self._do_raise(cs, raise_add)
                    
            else: # winRate > 65%
                if amount > pot_minus_amount * 4.0:
                    return ActionFold() if cs.can_act(ActionFold) else ActionCheck()
                else:
                    raise_add = (pf_edge * 3.0) * opp_agg_discount * pot 
                    return self._do_raise(cs, raise_add)

        elif cs.street == "flop":
            if winRate < 0.65: # Optimized 0.65
                if winRate >= 1.4 * (amount / (amount + pot + 0.0001)):
                    return ActionCall() if cs.can_act(ActionCall) else ActionCheck()
                else:
                    return ActionFold() if cs.can_act(ActionFold) else ActionCheck()
                    
            elif winRate <= 0.75: # Optimized 0.75
                raise_add = (post_edge * 2.0) * opp_agg_discount * pot 
                return self._do_raise(cs, raise_add)
                
            elif winRate > 0.90:
                raise_add = (post_edge * 3.0) * opp_agg_discount * pot 
                return self._do_raise(cs, raise_add)
                
            else: # 75% < winRate <= 90%
                raise_add = (post_edge * 2.5) * opp_agg_discount * pot 
                return self._do_raise(cs, raise_add)

        elif cs.street == "turn":
            if winRate < 0.68: # Optimized 0.68
                if winRate >= 1.5 * (amount / (amount + pot + 0.0001)):
                    return ActionCall() if cs.can_act(ActionCall) else ActionCheck()
                else:
                    return ActionFold() if cs.can_act(ActionFold) else ActionCheck()
            else:
                raise_add = (post_edge * 2.5) * opp_agg_discount * pot 
                return self._do_raise(cs, raise_add)

        elif cs.street == "river":
            if winRate < 0.70: # Optimized 0.70
                if winRate >= 1.5 * (amount / (amount + pot + 0.0001)): # Optimized 1.5
                    return ActionCall() if cs.can_act(ActionCall) else ActionCheck()
                else:
                    return ActionFold() if cs.can_act(ActionFold) else ActionCheck()
                    
            elif winRate > 0.92: # Optimized 0.92  
                raise_add = (post_edge * 3.5) * opp_agg_discount * pot 
                return self._do_raise(cs, raise_add)
                
            else: # 70% <= winRate <= 92%
                raise_add = (post_edge * 2.0) * opp_agg_discount * pot 
                return self._do_raise(cs, raise_add)

        return ActionCall() if cs.can_act(ActionCall) else ActionCheck()

    def _get_base_move(self, game_info: GameInfo, cs: PokerState):
        if cs.street == "auction":
            return ActionBid(self._bid(cs))

        if cs.street == "flop" and not self._opp_won_auction and not self._we_won_auction:
            if cs.opp_revealed_cards:
                self._we_won_auction = True
            else:
                self._opp_won_auction = True

        # === EXPLOIT A: Unconditional Pot-Sized Bluff ===
        # If we won the auction and the opponent overfolds (>40%), 
        # unconditionally bet 100% of the pot to force their facing-bet discounts.
        #if cs.street != "pre-flop" and self._we_won_auction:
        #    if self.opp_fold_pct > 0.40:
        #        if cs.can_act(ActionRaise):
        #           return self._do_raise(cs, cs.pot)

        eq = self._equity(cs)

        action = self._get_action(cs, eq)
        
        # Validating action against limits
        if isinstance(action, ActionRaise):
            if not cs.can_act(ActionRaise):
                action = ActionCall() if cs.can_act(ActionCall) else ActionCheck()
            else:
                mn, mx = cs.raise_bounds
                if action.amount < mn or action.amount > mx:
                    action = ActionRaise(max(mn, min(mx, action.amount)))
        elif isinstance(action, ActionCall):
            if not cs.can_act(ActionCall):
                action = ActionCheck()
        elif isinstance(action, ActionFold):
            if not cs.can_act(ActionFold):
                action = ActionCheck()

        return action

    def get_move(self, game_info: GameInfo, cs: PokerState):
        """Wrapper to evaluate and intercept actions from _get_base_move.
        Enforces raise caps per street and a minimum fold threshold.
        """
        # Street change detection for resetting raise counters
        if cs.street != self.current_street:
            self.current_street = cs.street
            self.raise_count = 0

        # Retrieve the underlying strategy's preferred move
        base_action = self._get_base_move(game_info, cs)

        # ------------------------------------------------------------------
        # RULE 1: Minimum Fold Threshold
        # Prevent the bot from folding when the cost to call is trivially low (<= 1000)
        # ------------------------------------------------------------------
        # if isinstance(base_action, ActionFold):
        #     if cs.cost_to_call <= 1000:
        #        return ActionCall() if cs.can_act(ActionCall) else ActionCheck()

        # ------------------------------------------------------------------
        # RULE 2: Raise Capping
        # Limit raises to a maximum of 2 per street UNLESS equity > 0.92
        # ------------------------------------------------------------------
        if isinstance(base_action, ActionRaise):
            # Check if this hand qualifies for the aggressive exception
            if self._equity(cs) > 0.92:
                self.raise_count += 1
                return base_action

            # Standard raise capping logic
            if self.raise_count >= 2:
                # Cap reached: convert raise to a call
                return ActionCall() if cs.can_act(ActionCall) else ActionCheck()
            else:
                # Cap not reached: allow raise and increment counter
                self.raise_count += 1
                return base_action

        return base_action

def run_simple_bot(pokerbot, args):
    assert isinstance(pokerbot, BaseBot)
    try:
        sock = socket.create_connection((args.host, args.port))
    except OSError:
        print('Could not connect to {}:{}'.format(args.host, args.port))
        return
    socketfile = sock.makefile('rw')
    runner = ExploitRunner(pokerbot, socketfile)
    runner.run()
    socketfile.close()
    sock.close()


if __name__ == "__main__":
    run_simple_bot(Player(), parse_args())

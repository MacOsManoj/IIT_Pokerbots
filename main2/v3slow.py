"""
v3slow.py — IIT Pokerbots 2026 "Sneak Peek Hold'em" Bot

SUPER-DEFENSIVE bot with exploit auction strategy.
Philosophy:
  - TIGHT preflop: fold marginal hands, only play strong holdings
  - SMALL postflop bets: control pot size, avoid bloating pots without nuts
  - HEAVY INFO EXPLOITATION: when we see opponent's card via auction,
    make PRECISE decisions — fold instantly when dominated, value-bet
    thin when they're weak, and never bluff into strength
  - CONSERVATIVE facing bets: fold more often when facing aggression
  - WIN by losing less: minimize losses on bad hands, extract max from good ones
"""
...
class Player(BaseBot):
    ...
    def _bid(self, cs: PokerState) -> int:
        """
        DEFENSIVE auction: keep BB bids conservative (save chips).
        SB exploit: same opp+1/opp-1/free-peek logic — this is where
        the real edge comes from, and it costs nothing to exploit.
        """
        ...
        # We'd rather lose the auction cheaply and let exploit handle SB
        # ══════════════════════════════════════════════════════════════
        if cs.is_bb:
            pot = cs.pot
    ...
    # ------------------------------------------------------------------
    # MAIN — SUPER DEFENSIVE with info exploitation
    # ------------------------------------------------------------------
    def get_move(self, game_info: GameInfo, cs: PokerState):
        ...
        # ── Minimal position boost (defensive = less positional aggression) ──
        if is_ip:
            equity += 0.01

        # ══════════════════════════════════════════════════════════════
        # INFORMATION EXPLOITATION (the core defensive edge)
        #
        # When we see their card, MC equity already accounts for it.
        # The REAL defensive value: we can FOLD precisely when their
        # revealed card connects with the board, and value-bet thin
        # when it doesn't. This saves us from paying off big hands.
        # ══════════════════════════════════════════════════════════════
        ...
        if self._we_won_auction and cs.opp_revealed_cards and cs.street != "pre-flop":
            ...
            if paired_board or flush_made:
                # Their revealed card hits the board — DANGER
                opp_card_strong = True
                equity -= 0.04  # Be more pessimistic
            elif high_card and flush_draw:
                opp_card_strong = True
                equity -= 0.02
            elif rev_rank <= 5 and not paired_board:
                # Their card is trash and missed the board
                opp_card_weak = True
                equity += 0.03  # We can safely value-bet thinner
            ...
            # They see OUR card — be more cautious
            equity -= 0.02

        # ── Texture & SPR adjustments (conservative) ──
        if tex == "wet" and not is_ip and 0.45 < equity < 0.65:
            equity -= 0.04  # Extra cautious OOP on wet boards
        if spr > 8 and 0.35 < equity < 0.55:
            equity += 0.01  # Minimal SPR boost

        equity = max(0.0, min(1.0, equity))

        # ==============================================================
        #  FACING-BET DISCOUNT — HEAVY (defensive core)
        #  We fold more to aggression. Only call with solid equity edge.
        # ==============================================================
        if to_call > 0 and cs.street != "pre-flop":
            bet_frac = to_call / (pot - to_call + 0.01)
            # Higher base discount than normal bots
            discount = min(0.16, bet_frac * 0.12)

            # If opponent won auction (has our info) — even more cautious
            if self._opp_won_auction:
                discount *= 1.6
                discount = min(discount, 0.22)

            # If we know their card is strong — bigger discount
            if opp_card_strong:
                discount += 0.04
        ...
        # ── Opponent exploitation (minimal — defensive bot doesn't bluff much) ──
        bluff_mod = 0.0
        if self.total_hands > 50:
            if self.opp_fold_pct > 0.50:
                bluff_mod = 0.04  # Only bluff very foldy opponents
            elif self.opp_fold_pct < 0.25:
                bluff_mod = -0.04  # Tighten up more vs calling stations
        ...
        #  - Smaller bet sizes across the board
        #  - Lower betting frequencies
        #  - Much faster folding to bets with marginal hands
        #  - Almost no bluffing unless we KNOW they're weak (info exploit)
        #  - When we have info + their card is weak → value bet precisely
        # ==============================================================

        # ── BUCKET 1: PREMIUM (equity > 0.82) — higher threshold ──
        if equity > 0.82:
            if cs.can_act(ActionRaise):
                # Small-medium bets to keep them in (extract value steadily)
                if opp_card_weak:
                    # They're weak — bet small to get called
        ...
        if equity > 0.68:
            if to_call > 0:
                # Facing bet with strong hand: just call (pot control)
                if cs.can_act(ActionCall):
                    return ActionCall()
                return ActionCheck()
            # No bet to face: bet small for value
            if cs.can_act(ActionRaise):
        ...
        if equity > 0.55:
            if to_call > 0:
                # Only call if we have clear pot odds advantage
                if pot_odds < equity - 0.06:
                    if cs.can_act(ActionCall):
                        return ActionCall()
                # Otherwise fold — don't chase
                if cs.can_act(ActionCheck):
                    return ActionCheck()
                return ActionFold() if cs.can_act(ActionFold) else ActionCheck()
            # Not facing a bet: small probe if info says they're weak
        ...
        if equity > 0.45:
            if to_call > 0:
                # Almost always fold marginal hands to bets (defensive!)
                ...
                if cs.can_act(ActionCheck):
                    return ActionCheck()
                return ActionFold() if cs.can_act(ActionFold) else ActionCheck()
            # Check through — control the pot
            return ActionCheck() if cs.can_act(ActionCheck) else ActionCall()

        # ── BUCKET 5: WEAK (equity 0.33 – 0.45) ──
        if equity > 0.33:
            if to_call > 0:
                # Fold to nearly all bets
                if cs.can_act(ActionCheck):
                    return ActionCheck()
                return ActionFold() if cs.can_act(ActionFold) else ActionCheck()
            # Check — only bluff if we KNOW they're super weak
        ...
        if cs.can_act(ActionCheck):
            # Almost never bluff — only if we know their card is absolute garbage
    ...
    def _pf_action(self, cs: PokerState, eq: float, is_ip: bool,
        ...
        if is_ip:
            if eq > 0.58:
                ...
                # Decent hands: limp/call
                if cs.can_act(ActionCall):
                    return ActionCall()
            else:
                # Fold everything below 50% — no speculative opens
                return ActionFold() if cs.can_act(ActionFold) else ActionCall()

        # BB (acts second preflop)
        else:
            if to_call == 0:
                # Not facing a raise: only raise with strong hands

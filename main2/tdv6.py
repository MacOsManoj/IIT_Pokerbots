...
# ═══════════════════════════════════════════════════════════════════════════════
# COUNTER-AGGRESSION CONFIGURATION
# Take initiative vs aggressive opponents instead of being a calling station
# ═══════════════════════════════════════════════════════════════════════════════
MANIAC_PFR_THRESHOLD = 0.45       # PFR >= 45% = maniac opponent
PREMIUM_4BET_MIN = 350            # Min 4-bet size with premiums
PREMIUM_4BET_MAX = 500            # Max 4-bet size with premiums
...
class Player(BaseBot):
    ...
    def _is_maniac_opponent(self) -> bool:
        """Check if opponent is classified as a maniac (PFR >= 45%)."""
        if self._hands_played < 10:
            return False
        pfr = self._opp_pfr_count / self._hands_played
        return pfr >= MANIAC_PFR_THRESHOLD
    ...
    def get_move(self, game_info: GameInfo, cs: PokerState):
        ...
        if cs.street == "pre-flop":
            ...
            elif opp_wager > 10 and cs.is_bb and not self._opp_vpip_this_hand:
                # Opponent is SB and completed to 20 (we are BB)
                self._opp_vpip_this_hand = True
    ...
    def _premium_action(self, cs: PokerState):
        ...
        # ── PREFLOP: Controlled raise (100-200 chips), or 4-bet vs maniacs ──
        if street == "pre-flop":
            # If we can raise and haven't faced a 3-bet yet, make standard raise
            if cs.can_act(ActionRaise):
                mn, mx = cs.raise_bounds
                
                # Check if facing a 3-bet (opponent raised after we raised, or large raise)
                facing_3bet = to_call > 150 and self._we_raised_this_hand
                facing_large_raise = to_call > 200
                
                if (facing_3bet or facing_large_raise) and self._is_maniac_opponent():
                    # 4-BET vs maniac: Don't just call, take the initiative back
                    raise_amt = random.randint(PREMIUM_4BET_MIN, PREMIUM_4BET_MAX)
                    ...
                    # Standard raise 5-10x BB (100-200 chips)
            ...
            # If facing a raise and can't re-raise, call if reasonable
            if to_call > 0:
        ...
        if to_call > 0:
            # Monster equity when FACING a bet: go for max value (defensive all-in)
        ...
        # Only go all-in on turn/river when checked to us (preserve probe on flop)
        allin_floor = self._get_dynamic_floor(ALLIN_FLOOR_MIN, ALLIN_FLOOR_MAX, street)
        if exact_eq >= allin_floor and street in ("turn", "river"):
            if cs.can_act(ActionRaise):
                _, mx = cs.raise_bounds
                self._we_raised_this_hand = True
                return ActionRaise(mx)
        
        # Use probe betting on all streets (including flop with monster equity)
    ...
    def _trash_action(self, cs: PokerState):
        ...
        if to_call > 0:
            # ═══════════════════════════════════════════════════════════════
            # FACING A BET: All-in allowed here (defensive) on any street
            # ═══════════════════════════════════════════════════════════════
            if exact_eq >= allin_floor:
                if cs.can_act(ActionRaise):
                    _, mx = cs.raise_bounds

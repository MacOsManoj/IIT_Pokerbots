"""
temp_drainv3.py — Adaptive Bot with Probe Bluffing (IIT Pokerbots 2026)

Based on temp_drainv2.py with added bluffing system.

Bluffing Strategy:
  - Tracks opponent fold-to-raise % (across all streets)
  - If opponent folds >=60% of time to our raises, activate probe bluffs
  - Bluff frequency scales: 0-20% of trash hands based on fold %
  - Probe bets: small escalating bets (30-50% pot) per street
  - Max bluff investment: 100 chips per hand
  - Bail triggers: fold if opponent re-raises, stop bluffing if called
  - Self-correcting: bluff bets feed back into fold tracker
"""
...
class Player(BaseBot):
    ...
    def _trash_action(self, cs: PokerState):
        """
        Trash hand strategy with DYNAMIC EQUITY FLOORS + PROBE BLUFFING:

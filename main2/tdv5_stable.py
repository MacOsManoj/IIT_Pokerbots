"""
tdv5.py — Adaptive Bot with STABILITY FOCUS (v5.1)

Based on tdv4.py with key changes:
  - Smaller bet sizes to get more calls (25-40% pot instead of 50-75%)
  - Premium hands use thin value (0.62+ threshold) instead of probe-bluff
  - Removed PREMIUM_BAD_BOARD_CUTOFF folding on premiums
  - Loosen thresholds against passive opponents using aggression factor
  - Simplified trash hand auction: SB exploit (opp_bid+1 <= 51), BB random 40-50
  - Removed bankroll target lock-in mode
  
  STABILITY (v5.1):
  - MAX_HAND_INVESTMENT cap (15% of starting stack = 750)
  - Reduced all-in floor thresholds (0.88-0.96)
  - Smaller premium preflop raises (60-100)
  - Reduced bluff investment (50 max)
  - Conservative passive opponent exploitation (max 1.25x)
"""
...
# ---------------------------------------------------------------------------
# STABILITY: Maximum investment per hand to reduce volatility
# ---------------------------------------------------------------------------
MAX_HAND_INVESTMENT = 750  # Never invest more than 15% of starting stack
MAX_SINGLE_BET = 400       # Cap any single bet/raise to reduce swings

# ---------------------------------------------------------------------------
# BASE thresholds (+20% from v3 for better pot odds calls)
# ---------------------------------------------------------------------------
BASE_PREFLOP_THRESHOLD = 96       # Base max chips to commit preflop with trash (was 80)
BASE_FLOP_THRESHOLD = 120         # Base max chips on flop (was 100)
BASE_TURN_THRESHOLD = 144         # Base max chips on turn (was 120)
BASE_RIVER_THRESHOLD = 168        # Base max chips on river (was 140)
...
# All-in floor (RAISED for stability - less frequent all-ins)
ALLIN_FLOOR_MIN = 0.88
ALLIN_FLOOR_MAX = 0.96
...
# ---------------------------------------------------------------------------
# BLUFFING CONFIGURATION (REDUCED for stability)
# ---------------------------------------------------------------------------
BLUFF_MIN_FOLD_PCT = 0.65       # Min fold % to activate bluffing (RAISED from 0.60)
BLUFF_MIN_SAMPLES = 12          # Min raise data points before bluffing (RAISED from 8)
BLUFF_MAX_FREQUENCY = 0.15      # Cap bluff frequency at 15% (REDUCED from 20%)
BLUFF_MAX_INVESTMENT = 50       # Max chips to risk per bluffing hand (REDUCED from 100)
BLUFF_POT_ABANDON = 150         # Stop bluffing if pot exceeds this (REDUCED from 200)

# Bluff bet sizing per street (fraction of pot) - REDUCED for more calls
BLUFF_BET_FRACTION = {
    "flop":  (0.20, 0.30),      # was (0.30, 0.40)
    "turn":  (0.25, 0.35),      # was (0.35, 0.45)
    "river": (0.30, 0.40),      # was (0.40, 0.50)
}

# ---------------------------------------------------------------------------
# PREMIUM THIN VALUE CONFIGURATION (REDUCED for stability)
# ---------------------------------------------------------------------------
PREMIUM_PREFLOP_RAISE_MIN = 60    # 3x BB (REDUCED from 100)
PREMIUM_PREFLOP_RAISE_MAX = 100   # 5x BB (REDUCED from 200)
PREMIUM_POSTFLOP_BET_MIN = 0.20   # was 0.25 - smaller for stability
PREMIUM_POSTFLOP_BET_MAX = 0.35   # was 0.40 - smaller for stability
PREMIUM_THIN_VALUE_THRESHOLD = 0.62  # Lower threshold for thin value betting

# ---------------------------------------------------------------------------
# POT ODDS CONFIGURATION
# ---------------------------------------------------------------------------
POT_ODDS_MARGIN = 0.05  # Conservative edge buffer for calling

# ---------------------------------------------------------------------------
# PASSIVE OPPONENT DETECTION (v5)
# ---------------------------------------------------------------------------
PASSIVE_AGGRESSION_THRESHOLD = 1.5  # Below this = passive opponent
PASSIVE_MIN_SAMPLES = 10             # Min samples for aggression factor
...
class Player(BaseBot):
    ...
    def __init__(self) -> None:
        ...
        # ═══════════════════════════════════════════════════════════════════
        # AGGRESSION FACTOR TRACKING (v5 - for passive opponent detection)
        # ═══════════════════════════════════════════════════════════════════
        self._opp_bets_and_raises = 0       # Opponent's aggressive actions
        self._opp_calls = 0                 # Opponent's passive actions
    ...
    def _get_aggression_factor(self) -> float:
        """
        Calculate opponent's aggression factor.
        AF = (bets + raises) / calls
        Higher = more aggressive, Lower = more passive
        """
        if self._opp_calls == 0:
            # If no calls, use a default based on bets/raises
            return float(self._opp_bets_and_raises) if self._opp_bets_and_raises > 0 else 1.0
        return self._opp_bets_and_raises / self._opp_calls
    ...
    def _update_street_thresholds(self) -> None:
        """
        Dynamically adjust street thresholds based on opponent's aggression factor.
        Against passive opponents, loosen thresholds to extract more value.
        """
        total_actions = self._opp_bets_and_raises + self._opp_calls
        
        if total_actions < PASSIVE_MIN_SAMPLES:
            return
        
        aggression_factor = self._get_aggression_factor()
        
        # Against passive opponents (low aggression factor), loosen thresholds
        # STABILITY: Reduced multipliers to avoid over-commitment
        if aggression_factor < PASSIVE_AGGRESSION_THRESHOLD:
            # Scale loosening based on how passive they are
            # AF 0.5 -> max loosening, AF 1.5 -> no loosening
            passivity = 1.0 - (aggression_factor / PASSIVE_AGGRESSION_THRESHOLD)
            passivity = max(0.0, min(1.0, passivity))
            
            # Preflop: up to 1.25x (max 140) - REDUCED from 1.5x/180
            preflop_mult = 1.0 + (0.25 * passivity)
            self._dynamic_preflop_threshold = min(140, int(BASE_PREFLOP_THRESHOLD * preflop_mult))
            
            # Flop: up to 1.2x (max 160) - REDUCED from 1.4x/200
            flop_mult = 1.0 + (0.2 * passivity)
            self._dynamic_flop_threshold = min(160, int(BASE_FLOP_THRESHOLD * flop_mult))
            
            # Turn: up to 1.15x (max 180) - REDUCED from 1.35x/220
            turn_mult = 1.0 + (0.15 * passivity)
            self._dynamic_turn_threshold = min(180, int(BASE_TURN_THRESHOLD * turn_mult))
            
            # River: up to 1.1x (max 190) - REDUCED from 1.3x/240
            river_mult = 1.0 + (0.1 * passivity)
            self._dynamic_river_threshold = min(190, int(BASE_RIVER_THRESHOLD * river_mult))
        else:
            # Against aggressive opponents, use standard thresholds scaled by raise amounts
            if len(self._opp_raise_amounts) >= 3:
                ...
                scale_factor = min(avg_raise / 100, 1.5)
    ...
    # ------------------------------------------------------------------
    # Auction logic for TRASH hands (v5 - simplified exploit)
    # ------------------------------------------------------------------
    def _bid(self, cs: PokerState) -> int:
        ...
        # BB case: simple random bid between 40-50
        if cs.is_bb:
            return min(random.randint(40, 50), cs.my_chips)

        # SB case: exploit with opp_bid+1 if <= 51
        ...
        # Simple SB exploit: only overbid if cheap enough
        if opp_bid + 1 <= 51:
            return min(opp_bid + 1, cs.my_chips)
        
        # Otherwise drain them by bidding less
    ...
    # ------------------------------------------------------------------
    # PREMIUM THIN VALUE BET (v5 - smaller sizing to get more calls)
    # ------------------------------------------------------------------
    def _get_premium_thin_value_bet(self, cs: PokerState, exact_eq: float) -> int:
        """Calculate a thin value bet for premium hands postflop.
        
        v5 change: Use smaller sizing (25-40% pot) to get more calls.
        Only bet if equity >= 0.62 (lower threshold than before).
        """
        pot = max(cs.pot, 1)
        
        # Only bet for thin value if equity is decent
        if exact_eq < PREMIUM_THIN_VALUE_THRESHOLD:
            return 0
        
        # Calculate thin value bet size: 25-40% pot (smaller to get calls)
    ...
    def get_move(self, game_info: GameInfo, cs: PokerState):
        ...
        # Track opponent aggression (for passive opponent detection)
        if cs.cost_to_call > 0:
            # Opponent bet or raised at us
            self._opp_bets_and_raises += 1
        ...
        # --- PREMIUM HAND: probe-then-bluff strategy (CHANGED in v4) ---
        if self._is_premium:
    ...
    def _premium_action(self, cs: PokerState):
        """
        THIN VALUE strategy for premium hands (JJ+) - v5 changes.
        
        Key changes from v4:
        1. Lower threshold to 0.62 for thin value betting
        2. Smaller bet sizes (25-40% pot) to get more calls
        3. Removed PREMIUM_BAD_BOARD_CUTOFF folding - don't fold premiums too much
        """
        street = cs.street
        ...
        # ── MONSTER EQUITY (88%+): Go for value but CAP to reduce variance ──
        allin_floor = self._get_dynamic_floor(ALLIN_FLOOR_MIN, ALLIN_FLOOR_MAX, street)
        if exact_eq >= allin_floor:
            if cs.can_act(ActionRaise):
                mn, mx = cs.raise_bounds
                # STABILITY: Cap raise to MAX_SINGLE_BET instead of all-in
                capped_raise = min(mx, MAX_SINGLE_BET, MAX_HAND_INVESTMENT - self._premium_invested)
                capped_raise = max(mn, capped_raise)
                self._we_raised_this_hand = True
                self._premium_invested += capped_raise
                return ActionRaise(capped_raise)
            if cs.can_act(ActionCall):
                return ActionCall()
            return ActionCheck()
        
        # ── FACING A BET: Use pot odds (no more bad board folding) ──
        if to_call > 0:
            if exact_eq >= required_eq:
                # Good pot odds - call or raise for value
                if exact_eq >= 0.70 and cs.can_act(ActionRaise):
                    # Strong equity: thin value raise with smaller sizing
                    mn, mx = cs.raise_bounds
                    raise_amt = int(cs.pot * random.uniform(0.25, 0.40))
                    raise_amt = max(mn, min(mx, raise_amt))
                    self._we_raised_this_hand = True
                    return ActionRaise(raise_amt)
                # Track opponent call for aggression factor
                self._opp_calls += 1
                return ActionCall()
            
            # Pot odds don't justify call - but be more sticky with premiums
            # Only fold if equity is really bad AND pot odds are terrible
            if exact_eq < 0.35:
                return ActionFold() if cs.can_act(ActionFold) else ActionCheck()
            
            # Otherwise call with premium - assume opponent doesn't bluff enough
            if cs.can_act(ActionCall):
                self._opp_calls += 1
                return ActionCall()
            return ActionCheck()
        
        # ── CHECKED TO US: Thin value bet with smaller sizing ──
        if exact_eq >= PREMIUM_THIN_VALUE_THRESHOLD:
            bet_amt = self._get_premium_thin_value_bet(cs, exact_eq)
    ...
    def _trash_action(self, cs: PokerState):
        ...
        if street == "pre-flop":
            ...
            if to_call <= threshold - cs.my_wager:
                ...
                if self._current_pf_eq >= required_eq:
                    if cs.can_act(ActionCall):
                        self._opp_calls += 1  # We called their bet
                        return ActionCall()
        ...
        # Strong value: if equity exceeds our dynamic floor (CAPPED for stability)
        if exact_eq >= allin_floor:
            if cs.can_act(ActionRaise):
                mn, mx = cs.raise_bounds
                # STABILITY: Cap raise to avoid massive swings
                capped_raise = min(mx, MAX_SINGLE_BET)
                capped_raise = max(mn, capped_raise)
                self._we_raised_this_hand = True
                return ActionRaise(capped_raise)
            if cs.can_act(ActionCall):
        ...
        # ── Normal value bet if we meet the floor (smaller sizing) ──
        if exact_eq >= value_bet_floor:
            pot = cs.pot
            # Smaller value bet sizing to get more calls
            bet_amt = int(pot * random.uniform(0.20, 0.35))

...
class GameState(namedtuple('_GameState', ['dealer', 'street', 'auction', 'bids', 'wagers', 'chips', 'hands', 'opp_hands', 'community_cards', 'parent_state'])):
    ...
    def next_street(self):
        ...
        if self.street == 0:
            # Auction should start with BB (who posted BIG_BLIND) bidding first
            # Determine BB by checking who has the larger initial wager
            bb_is_player_1 = self.wagers[1] > self.wagers[0]
            starting_dealer = 1 if bb_is_player_1 else 0
            return GameState(starting_dealer, 3, True, self.bids, [0, 0], self.chips, self.hands, self.opp_hands, self.community_cards, self)    

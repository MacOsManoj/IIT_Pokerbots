...
int main() {
    ...
    for (int r1 = 12; r1 >= 0; --r1) { // Ranks: 12=Ace down to 0=2
        for (int r2 = r1; r2 >= 0; --r2) { // r1 >= r2 prevents duplicates
            if (r1 == r2) {
                // Pocket Pair (e.g. As Ah)
                int c1 = r1 * 4 + 0;
                int c2 = r1 * 4 + 1;
    ...
    for (size_t h = 0; h < distinct_hands.size(); ++h) {
        ...
        mt19937 rng(1337 + omp_get_thread_num() + h); 
        int hero_wins = 0;
        int ties = 0;
    ...
    // ---------------------------------------------------------
    // WRITE OUTPUT TO TEXT FILE (IN PYTHON DICT FORMAT)
    // ---------------------------------------------------------
    string filename = "preflop_equities.txt";
    ofstream outfile(filename);

    if (outfile.is_open()) {
        outfile << "{\n";
        for (size_t i = 0; i < distinct_hands.size(); i++) {
            outfile << "    \"" << distinct_hands[i].name << "\": " 
                    << fixed << setprecision(5) << distinct_hands[i].equity;
            
            // Add a comma if it's not the last element
            if (i != distinct_hands.size() - 1) {
                outfile << ",";
            }
            outfile << "\n";
        }
        outfile << "}\n";
        outfile.close();
        cout << "SUCCESS: Saved all 169 hands to " << filename << " as a Python dictionary.\n";
    } else {

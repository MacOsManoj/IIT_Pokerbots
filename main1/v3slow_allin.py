"""
v3slow_allin.py — v3slow that goes ALL-IN with WEAK hands

Based on v3slow.py defensive bot (70% win rate) with one key modification:
  When equity < 0.45 (weak/trash hands), GO ALL-IN instead of folding.
  
This creates a polarized strategy:
  - Strong hands (equity > 0.45): Play defensive v3slow style
  - Weak hands (equity < 0.45): Aggressive all-in bluffs

The idea is to put maximum pressure with trash hands, making it hard
for opponents to exploit us when we show weakness. If they fold, we
win the pot. If they call, we still have some equity with our trash.
"""
...
# ======================== ALL-IN THRESHOLD ========================
WEAK_HAND_THRESHOLD = 0.45  # Below this equity → go all-in (bluff)
...
class Player(BaseBot):
    ...
    # ------------------------------------------------------------------
    # MAIN — v3slow defensive logic + ALL-IN WITH WEAK HANDS
    # ------------------------------------------------------------------
    def get_move(self, game_info: GameInfo, cs: PokerState):
        ...
        # ==============================================================
        #  ★★★ CORE MODIFICATION: GO ALL-IN WITH WEAK HANDS ★★★
        #
        #  If equity < 0.45, instead of checking/folding,
        #  we go ALL-IN to put maximum pressure on opponent.
        #  This creates a polarized range: nuts or air.
        # ==============================================================
        if equity < WEAK_HAND_THRESHOLD:
            # Go all-in with weak hands (bluff shove)
            if cs.can_act(ActionRaise):
                _, max_raise = cs.raise_bounds
                return ActionRaise(max_raise)  # ALL-IN
        ...
        # ==============================================================
        #  POSTFLOP BUCKETS — DEFENSIVE (identical to v3slow for equity >= 0.45)
        # ==============================================================
        ...
        # This is the lowest bucket now — everything < 0.45 goes all-in above
        if equity > 0.45:
            if to_call > 0:
        ...
        # Shouldn't reach here (equity < 0.45 handled above)

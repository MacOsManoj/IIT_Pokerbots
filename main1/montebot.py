...
class Player(BaseBot):
    ...
    def estimate_peek_ev(self, equity, current_state):
        """Information value based on uncertainty. Bid aggressively around equity ~0.5."""
        pot = current_state.pot
        uncertainty = 1.0 - abs(equity - 0.5) * 2.0  # peaks at 1.0 when eq=0.5
        info_value = uncertainty * pot * 0.22  # slightly above opponent's 15-20%
        if 0.4 < equity < 0.6:
            info_value *= 1.25  # boost for truly marginal spots
    ...
    def get_move(self, game_info: GameInfo, current_state: PokerState):
        ...
        if equity > 0.62:
            if current_state.can_act(ActionRaise) and r < 0.50:
                min_r, max_r = current_state.raise_bounds
                bet = int(pot * 0.66)
        ...
        if equity > 0.48:
            ...
            if current_state.can_act(ActionCheck):
                return ActionCheck()
            # Small bluff raise when IP
            if r < 0.12 and is_ip and current_state.can_act(ActionRaise):
        ...
        if r < 0.07 and current_state.can_act(ActionRaise):
            min_r, max_r = current_state.raise_bounds
            bet = int(pot * 0.75)
            return ActionRaise(min(max_r, max(min_r, bet)))
        return ActionFold() if current_state.can_act(ActionFold) else ActionCheck()
